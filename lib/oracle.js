...
exports.initialize = function initializeDataSource(dataSource, callback) {
  ...
  if (oracleSettings.useUTC === undefined) {
    oracleSettings.useUTC = true; // Default to true
  }
...
function dateToOracle(val, dateOnly, useUTC) {
  function fz(v) {
    return v < 10 ? '0' + v : v;
  }
  ...
  if (useUTC === true) {
    ...
    var dateStr =
      [val.getFullYear(), fz(val.getMonth() + 1), fz(val.getDate())].join('-') +
      ' ' +
      [fz(val.getHours()), fz(val.getMinutes()), fz(val.getSeconds())].join(
        ':',
      );
  }
  console.log(useUTC, dateStr);
  if (!dateOnly) {
...
Oracle.prototype.toColumnValue = function(prop, val) {
  ...
  if (prop.type === Date || prop.type.name === 'Timestamp') {
    return dateToOracle(val, prop.type === Date, this.settings.useUTC);
  }
